...
> 总结:  里氏替换原则就是子类可以扩展父类的功能,但不能覆盖父类的原有的功能 具体查看[[1. 设计模式#^4ac6a9|  里氏替换原则实现规则]] , 实现方式通过接口和抽象类实现
...
> 总结: 依赖倒转原则本质上就是将程序中硬编码的部分进行解耦,主要应用的设计模式为: 工厂设计模式,实现方式通过工厂设计模式加上配置类对程序中硬编码的部分进行替换
