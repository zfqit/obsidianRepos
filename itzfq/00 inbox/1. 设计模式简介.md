...
* 注意事项和细节
	* 继承关系导致父类和子类的耦合性增强了,在适当的情况下可以使用 **聚合**.**组合**.**依赖**来解决问题
...
> 总结:  里氏替换原则就是子类可以扩展父类的功能,但不能覆盖父类的原有的功能 具体查看[[1. 设计模式简介#^4ac6a9| 里氏替换基本概念]] ,在适当的情况下可以使用 **聚合**.**组合**.**依赖**来解决继承耦合性高的问题
...
```
通过对象之间的组合.依赖.聚合,我们就大大降低了类之间的耦合度，并且A的实现细节我们也不会直接得到了。
